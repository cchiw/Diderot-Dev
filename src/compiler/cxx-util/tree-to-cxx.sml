...
structure TreeToCxx : sig
   ...
   *)
    val loadNrrd : CLang.exp * CLang.exp -> CLang.stm
  ...
  (* Translate a TreeIR operator application to a CLang expression *)
    fun trOp (env, rator, cxxArgs) = (case (rator, cxxArgs)
           of (Op.IAdd, [a, b]) => CL.mkBinOp(a, CL.#+, b)
            ...
            | (Op.VClamp(w, _), [a, b, c]) => CL.mkApply(RN.vclamp w, [a, b, c])
            | (Op.VMapClamp(w, _), [a, b, c]) => CL.mkApply(RN.vclamp w, [a, b, c])
            | (Op.VLerp(w, _), [a, b, c]) => CL.mkApply(RN.vlerp w, [a, b, c])
            ...
            | (Op.VToInt(w, _), [a]) => CL.mkApply (RN.vtoi w, [a])
...
(* FIXME: eventually we should do some kind of liveness analysis to enable in situ operations *)
	       ...
	       | (Op.GetTracker, [a])  => CL.mkApply("GetTracker", [a])
            ...
            | (Op.NodeToCoord, [a])  =>  CL.mkApply("NodeToCoord1", [a])
            | (Op.Coordinates, [a])  =>  CL.mkApply("Coordinates1", [a])
         
            | (Op.Dimension, [a, b, c])  =>  CL.mkApply("Dimension3", [a, b, c])
            | (Op.NumCells, [a, b, c])  => CL.mkApply("NumCells3", [a, b, c])
            | (Op.BasisData, [a, b, c])  =>  CL.mkApply("BasisData3", [a, b, c])
            | (Op.CellToNode, [a, b, c])  =>  CL.mkApply("CellToNode3", [a, b, c])
            | (Op.NodeToPoint, [a, b, c])  =>  CL.mkApply("NodeToPoint3", [a, b, c])
            | (Op.NodeToCoord, [a, b, c])  =>  CL.mkApply("NodeToCoord3", [a, b, c])
            | (Op.Coordinates, [a, b, c])  =>  CL.mkApply("Coordinates3", [a, b, c])
            ...
            | (Op.GetCell, [findcell, basisEval])  =>  CL.mkApply("GetCell", [findcell])
            | (Op.GetPos, [findcell, basisEval])  =>  CL.mkApply("GetPos", [findcell])
            | (Op.GetNode, args)  =>  CL.mkApply("GetNode", args)
            | (Op.ProbeNodeB, args)  =>  CL.mkApply("ProbeNodeB", args)
              ...
              (* if necessary, wrap the argument so that the correct "<<" instance is used *)
                val e = (case ty
                       of Ty.TensorTy shape => CL.mkApply(RN.tensorRefStruct shape, [e])
                        | Ty.BoolTy => CL.mkCond(e, CL.mkStr "true", CL.mkStr "false")
                        | _ => e
                      (* end case *))
                in
          ...
          fun trStm (stm, (env, stms : CL.stm list)) = (case stm
                 of IR.S_Comment text => (env, CL.mkComment text :: stms)
                  ...
                  | IR.S_LoadNrrd(lhs, ty, nrrd) => let
                      val stm = loadNrrd (lvalueVar (env, lhs), CL.mkStr nrrd)
                      in
                        (env, stm :: stms)
                      end
                  ...
                  | IR.S_Return e => (env, CL.mkReturn(SOME(trExp(env, e))) :: stms)
