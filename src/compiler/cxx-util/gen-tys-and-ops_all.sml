...
*)
			...
			mkFunc(
              ...
              | translateCoordinates (space, dim, avgPos, basisFunctions, basisJacobian) =>
		let
                
                    val _ = print("\ntranslate coord():"^Int.toString(dim))
                 
		    (*basisFunctions is gBasisFunctions and basisJacobian is gBasisJacobian*)
		    
		    val prefix = CxxNames.mkFemPrefix(space)
						       
		    val basisFunctionsLegnth = List.length basisFunctions
							   
		    val rows = List.length basisJacobian (*number of basis functions*)
		    val col = List.length (List.hd basisJacobian) (*This is the geometric dimension, gdim*)
					  
		    (*Now we can write makeTransform. We start with some more matrix utiliuties specalized to thsi particular case*)
		    (*These are useful things to itterate over*)
		    val rowAcc = List.tabulate(dim, fn x => x)
		    val dimDimTuples = List.concat (List.tabulate(dim, fn x => (List.map (fn y => (x,y)) rowAcc)))
		    val colAcc = List.tabulate(basisFunctionsLegnth, fn x => x)
		    ...
		    val makeTransformHelper = GF.makeTransformHelper{prefix=prefix, dim=dim, rowAcc=rowAcc, basisFunctions=basisFunctions}
		    ...
		    (*we can now make makeTransform itself via the maps:nM,pM and cell. Manipulation of these is handled through getPoint, which is handled femptr.hxx*)
                    (* Note does not seem to be used anymore *)
		    (*val makeTransform = GF.makeTransform{prefix=prefix, dim=dim}*)
							
		    (*we now do makeJacobian in a similiar fashion*)
		    (*val helpMakeJacobian = GF.helpMakeJacobian{prefix=prefix, dim=dim, colAcc=colAcc, basisJacobian=basisJacobian, dimDimTuples=dimDimTuples}*)
							      
		    (*we from the inline hlper to the real thing again:*)
		    (*val makeJacobian = GF.makeJacobian{prefix=prefix,dim=dim}*)
						      
		    val helpInverseJacobian = GF.helpInverseJacobian{prefix=prefix, dim=dim, colAcc=colAcc, basisJacobian=basisJacobian, dimDimTuples=dimDimTuples}
								    
		    (*wrap up all of the rest into a functon you might call with a cell and the relevant maps*)
		    (*val jacobianInverse = GF.jacobianInverse{prefix=prefix,dim=dim}*)
							    
		    (*make a function specalized to the evaluation*)

		    val helpJIs = GF.helpJIs{prefix=prefix,dim=dim, rowAcc=rowAcc, avgPos=avgPos, helpInverseJacobian=helpInverseJacobian}
		    ...
		    val helpTranslateCoordinates = GF.helpTranslateCoordinates{prefix=prefix, dim=dim, rowAcc=rowAcc, avgPos=avgPos}
		...
		in
		    CL.D_Verbatim ret
              ...
              | findCellExpand  (space as meshElem.Space(mesh,element, degree), testString, isAffineTF,dim,gdim) =>
		let
		    val prefixM = CxxNames.mkFemPrefix(space)
		    val prefixME = CxxNames.mkFemPrefix space
		    val isAffine = (isAffineTF=1)
		    val wrapCell = GF.wrapCell{prefixM=prefixM, prefixME=prefixME, dim=dim,
					       testString=testString, isAffine=isAffine,gdim=gdim}
		    val ret = List.concat( [wrapCell])
		in
		    CL.D_Verbatim ret
		end
              | basisEvaluation  (space as  meshElem.Space(mesh,_,_), sBasisFunctions,level,sBasisDervs) =>
		let
		    
		    val prefixME = CxxNames.mkFemPrefix space
		    val dim = ME.dimOfMesh mesh
		    val numEvalbasis = List.length sBasisFunctions (*also know as sdim*)
		    val sBasisItter = List.tabulate( numEvalbasis,fn x => x)
		    (* val makeBasisEvaluation =  GF.makeBasisEvaluation{prefixME=prefixME, sBasisFunctions=sBasisFunctions, sBasisItter=sBasisItter} (*Potential replication issues here?*) *)
		    val makeEvaluations = if level = 0
				       then GF.makeBasisEvaluation{prefixME=prefixME, sBasisFunctions=sBasisFunctions, sBasisItter=sBasisItter} (*Potential replication issues here?*)
				       else GF.makePhiDerv({prefix=prefixME,sdim=numEvalbasis,dim=dim,dlevel=level,s_derivativeInfoList=sBasisDervs})
		    val ret = makeEvaluations
		in
		    CL.D_Verbatim ret
		end
		    
              | ProbePhi  (space as meshElem.Space(_,_,_), nnodes, dx, dy, dz) =>
		let
		    val prefixME = CxxNames.mkFemPrefix space
		...
		in
		    CL.D_Verbatim ret
              ...
              | EvalFem (space as meshElem.Space(mesh,_,_),sdim,dlevel,shape)=>
		let
		    val prefixME = CxxNames.mkFemPrefix space
		    val dim = ME.dimOfMesh mesh
                    
                    val _ = print("\ngiving EvalFem:dim"^Int.toString(dim)^"sdim:"^Int.toString(sdim)^"\n")
                                
		    val affineData = if dlevel = 0
				     then []
				     else GF.derivativeAffineCase({prefix=prefixME,sdim=sdim,dim= dim, dlevel=dlevel})
		    val makeEvaluate = GF.makeEvaluate({prefixME=prefixME,sdim=sdim,dim=dim,dlevel=dlevel,shape=shape,
							namespace=namespace})
		    val ret = List.concat( [affineData,makeEvaluate])
