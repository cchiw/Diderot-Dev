...
(* end case*))
in return (E.If(comp, E.Probe(e3, x), E.Probe(e4, x))) end
              | _                  => E.Probe(fld, x)
            (* end case *)
(*
            val _ = (String.concat["\n\n ** mkprobe", EinPP.expToString(E.Probe(fld, x)), "->", EinPP.expToString(pp)])*)
            in pp
          end
...
(*
        val _ = (String.concat["\n\n\n inside transform", EinPP.expToString(body)])*)
...
(*
                ...
                (************** Field Terms **************)
                  ...
                  | E.Probe(e1, e2)               => mkProbe (rewrite e1, rewrite e2)         (*should be mkprobe*)
                (************** Field Terms **************)
...
| E.Opn(E.Prod, (eps1 as E.Epsilon(i,j,k))::ps) => ((case ps
                       of ((p1 as E.Apply(E.Partial (d as (_::_::_)), e)) :: es) => (
                        ...
                        | _  => let
                            val body'=(case EpsUtil.epsToDels (eps1::ps)
                                 of (SOME(e, sx), NONE, rest) =>
                                    (* Changed to Deltas*)
                                    (case (sx, rest)
of ([], []) => (ST.tick cntEpsToDeltas;e)
| ([], _)   => (ST.tick cntEpsToDeltas;  E.Opn(E.Prod, e::rest))
                                        | (_, [])   => (ST.tick cntEpsToDeltas; E.Sum(sx, e))
                                        | (_, _)    => (ST.tick cntEpsToDeltas; E.Opn(E.Prod, E.Sum(sx, e)::rest))
                                    (* end case *))
                            ...
                            in body' end
                      (* end case *)))
                  | E.Opn(E.Prod, (d as E.Delta( _,  _ ))::es) => (case es
                       of [E.Op1(E.Neg, e1)] => (
                            ST.tick cntNegDelta; E.Op1(E.Neg, mkProd[d, e1]))
                        | _ => let
                            val (pre', eps, dels, post) = EinFilter.partitionGreek(d::es)
                            in
                              case EpsUtil.reduceDelta(eps, dels, post)
                                of (false, _) => let
                                 
                                    val e1 = rewrite (d)
                                    in (case (rewrite(mkProd es))
                                        of E.Opn(E.Prod, p1) => mkProd (e1::p1)
                                        |  p1                => mkProd [e1, p1]
                                        (*end case*))
                                    end
                ...
                (************* Product Generic **************)
                  ...
                  | E.Opn(E.Prod, [e1, e2]) => E.Opn(E.Prod, [rewrite e1, rewrite e2])
...
val e1' = rewrite e1
                      val newbie =  (case es
                            of [] => (ST.tick cntFilter ;e1')
                            | [e2'] => (case rewrite e2'
of E.Opn(E.Prod, es') => (ST.tick cntFilter ;E.Opn(E.Prod, e1'::es'))
                                | e2' => (E.Opn(E.Prod, [e1',e2']))
                                (* end case *))
                            | _ =>
                                (case (rewrite (E.Opn(E.Prod, es)))
                                    of E.Opn(E.Prod, es') => E.Opn(E.Prod, e1'::es')
                                    | e2' => (ST.tick cntFilter ;E.Opn(E.Prod, [e1',e2']))
                                    (* end case *))
                            (* end case *))
  (* debug  val _ =  (String.concat["\n\tpproduct rewrtie:",EinPP.expToString(body),"\n\t\t->:",EinPP.expToString(newbie )])*)
                ...
                (* end case *))
            (* debug*)
            val _ =  (String.concat["\n\trewrite:",EinPP.expToString(body),"\n\t\t->:",EinPP.expToString(bb)])
        in
            bb
        end
(*DEBUG*)val start = ST.count cntRounds
          fun loop (body, total, changed) = let
                ...
                (* debug   val _ =(String.concat["\n\n", Int.toString(total)," ==> X:", EinPP.expToString(body),"\n ==> Y:", EinPP.expToString(body')])*)
