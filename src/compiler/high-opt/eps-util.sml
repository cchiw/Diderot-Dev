...
structure EpsUtil : sig
    ...
    fun distributeEps (epsAll, sumexp) = let
          fun distEps ([], _) = NONE
            | distEps ([e1], eps) = (case sumexp
                (* Check the embedded eps for a match
                 * If found then return sum_ids and the rest of the product
                 *)
                 of _ => NONE
                (* end case *))
 ...
 (*
 val _ = (String.concat[" \n\nEPSALLL:", EinPP.expToString(E.Opn(E.Add, epsAll))])
  val _ = (String.concat["\n\n rest:", EinPP.expToString(E.Opn(E.Add, epsAll))])
  val _ = (String.concat["\n \nsumexp:", EinPP.expToString(E.Opn(E.Add, epsAll))])
 *)
 in
 ...
 fun setDelta e = E.Delta e
 fun setProd e= E.Opn(E.Prod, e)
