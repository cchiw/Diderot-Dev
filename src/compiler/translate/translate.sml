         ...
         *)
          ...
          fun doVar (y, (env', phiMap)) = (case VMap.find(env, y)
                 of SOME y' => let
                      val y'' = newVar y
                      in
                        (VMap.insert(env', y, y''), VMap.insert(phiMap, y, (y'', [y', y''])))
                      end
...
*)
                      ...
                      (VMap.insert (env, srcVar, dstVar), (dstVar, filterPhiRHS srcVars)::phis))
...
*)
                      ...
                      (VMap.insert (env, srcVar, dstVar), (dstVar, xs)::phis)
                    end
              val (env, phis') = VMap.foldli doVar (env, []) (!phiMap)
              in
                phis := phis';
    ...
    fun vToString(title, v) = (print (concat["\n",title,":", IR.Var.toString(v)]);v)
    fun nToString(title, n) = (print (concat["\n",title,":", IR.Node.toString(n)]))
    (*gets first variable it sees *)
    fun get_first_var (nd as IR.ND{kind, ...}) =
        (case kind
            of IR.ASSIGN{stm as (v, _),...} =>  vToString("Assign", v)
            | IR.EXIT {pred, succ, kind as ExitKind.RETURN(SOME v)} => vToString("Exit", v)
            | IR.JOIN {preds, succ, ...} =>  get_first_var(!succ)   
            | IR.ENTRY {succ, ...}   =>  get_first_var(!succ)
            | _ => raise Fail (concat["uncovered node kind: ", IR.Node.toString(nd)])
        (* end case*))
        
	fun gatherES(lhs_comp, node) = let
	
	    (*gather : node->  variable option * assignment list 
	    * variable option is the exit variable 
	    *)
	...
	fun errmsg(ins,alphas) = concat["Field is probed with arguments that do not match the vector sizes indicated by the field dimension.",
...
(*
               (* when the variable is inside CONS it needs to be broken apart for cfe *)
              let
...
*)
            ...
            | S.E_FieldFn f => let
					...
					val lhs_comp =  get_first_var(!pred)
            		(*get all the statements used in the function body *) 
            		val (_, stmt_comp) = gatherES (lhs_comp, entry)     		
            		(*analyze parameters*)	              	
					val lhs_PF = paramsF
					val lhs_PT = paramsT
            		...
            		* here checks that input type in ofield matche probed terms*)
            		val ins = (case  IR.Var.ty lhs
            			...
            			| HighTypes.FieldTy => alphas_allP
            			(*end case*))				
					fun match([],[]) = true 
					  | match(e1::es, p1::ps) = 
						if(e1=p1) then match(es,ps) else raise Fail(errmsg(ins, alphas_allP))
                     | match _ = raise Fail(errmsg(ins,alphas_allP))										
					val _ = match(ins, alphas_allP)					    				
            		(*create ein operator*)
					val rator  = MkOperators.cfexpMix (alpha_comp, alphas_PF,alphas_PT)   
					val args =  lhs_comp::lhs_allP 
					val ein = IR.EINAPP(rator,args)
                    val stmts = stmt_allP@ stmt_comp@[IR.ASSGN(lhs, ein)]
 ...
 *)
                  ...
                  | S.S_Foreach(x, xs, b) => let
                     ...
                     *)
                      ...
                      val (body, _) = cvtBlock (state, VMap.insert(env, x, x'), (1, join)::joinStk, b)
...
(* FIXME: the following code doesn't work properly if the exit node isn't reached *)
...
(*DEBUG*)handle ex => (print "error in cvtMethod\n"; raise ex)
  ...
  (* convert global code *)
    fun cvtGlobalBlock block = let
        (* load the globals into fresh variables *)
          val (loadCFG, env) = loadGlobals (VMap.empty, block)
        (* convert the code *)
...
(*DEBUG*)handle ex => raise ex
  ...
  (* extend the global environment with the strand's parameters *)
    fun initEnvFromParams params = let
          ...
          val (env, params) = List.foldl cvtParam (VMap.empty, []) params
          in
            (env, List.rev params)
          end
  ...
  (* lift functions used in map-reduce expressions *)
    ...
    fun translate prog = let
          ...
          val constInit = let
                val (cfg, _) = cvtBlock (([], []), VMap.empty, [], constInit)
                val cfg = IR.CFG.prependNode (IR.Node.mkENTRY(), cfg)
                val cfg = IR.CFG.appendNode (cfg, IR.Node.mkRETURN NONE)
                in
                  cfg
        ...
        (* create the global initialization code *)
          val globInit = let
               ...
               * other globals
               *)
                val (loadBlk, env) = loadGlobals (VMap.empty, globInit)
              ...
              (* convert the state initialization code *)
                val (stateInit, env) = let
                    (* load globals into local variables *)
                      ...
                      val env = List.foldl (fn (x, env) => VMap.insert(env, x, newVar x)) env state
                ...
                fun cvtMeth isStabilize blk =
                      cvtMethod (env, isStabilize, state, svars, blk)
                in
                  IR.Strand{
                      name = name,
                      params = params,
...
(*DEBUG*)handle ex => raise ex
          val create = Create.map cvtGlobalBlock create
          val prog = IR.Program{
                  props = props,
                  consts = consts',
                  inputs = inputs',
                  ...
                  create = create,
                  start = Option.map cvtGlobalBlock start,
                  update = Option.map cvtGlobalBlock update
                }
          in
            Census.init prog;
            prog
