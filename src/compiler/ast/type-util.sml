 ...
 *)
 ...
 (* return true if the type is an Fem type *)
 val isMeshType : Types.ty -> bool
   ...
   *)
    fun prune ty = (case ty
           of (ty as Ty.T_Var(Ty.TV{bind, ...})) => (case !bind
            ...
            | Ty.T_Mesh => Ty.T_Mesh
            | Ty.T_Element=> Ty.T_Element
            | Ty.T_FnSpace => Ty.T_FnSpace
  ...
  (* prune the head of a type *)
    fun pruneHead ty = let
          fun prune' (ty as Ty.T_Var(Ty.TV{bind, ...})) = (case !bind
            ...
            | prune' Ty.T_Mesh  = Ty.T_Mesh
            | prune' Ty.T_Element= Ty.T_Element
            | prune' Ty.T_FnSpace = Ty.T_FnSpace
            | prune' ty = ty
          in
            prune' ty
          end
  ...
  (* equality testing *)
    ...
    fun toString ty = (case pruneHead ty
           of Ty.T_Var(Ty.TV{bind=ref(SOME ty), ...}) => toString ty
            ...
            | Ty.T_Mesh => "meshTy"
            | Ty.T_Element => "Element"
            | Ty.T_FnSpace => "FnSpace"
          (* end case *))
    ...
    fun instantiate ([], ty) = ([], ty)
      | instantiate (mvs, ty) = let
          ...
          fun ity (Ty.T_Var tv) = (case MV.Map.find(env, Ty.TYPE tv)
            ...
            | ity Ty.T_Element= Ty.T_Element
            | ity Ty.T_FnSpace = Ty.T_FnSpace
