  ...
  (* convert a Types.ty to a SimpleTypes.ty *)
    fun cvtTy ty = (case ty
           of Ty.T_Var(Ty.TV{bind, ...}) => (case !bind
            ...
            | Ty.T_Mesh => STy.T_Mesh
            | Ty.T_Element=> STy.T_Element
            | Ty.T_FnSpace => STy.T_FnSpace
                  ...
                  in
                    case Var.kindOf f
                     of Var.FunVar => (stms, S.E_Apply(SimpleFunc.use(cvtFunc f), xs))
                      | Var.InputVar =>  (stms, S.E_Apply(SimpleFunc.use(cvtFunc f), xs))
                      | e => raise Fail ("bogus application"^Var.kindToString(f))
                    (* end case *)
                  end
