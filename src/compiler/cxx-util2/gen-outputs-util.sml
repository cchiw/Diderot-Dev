...
(* FIXME: use biffMsgAddF and return error status *)
          fun error [] = CL.mkCall("exit", [CL.mkInt 1])
            | error msg = CL.mkBlock[
	 ...
	 * a variable name and optional annotation.
	 *)
	  fun mkOutFile (name, NONE) = if snapshot
		then CL.mkVar (snapstem (isMultiOutput, name))
		  ++ CL.mkVar "suffix"
		  ++ CL.mkStr ".nrrd"
		else CL.mkVar (outstem (isMultiOutput, name))
	    | mkOutFile (name, SOME kind) = if snapshot
		then CL.mkVar (snapstem (isMultiOutput, name))
		  ++ CL.mkStr kind
		  ++ CL.mkVar "suffix"
		  ++ CL.mkStr ".nrrd"
          ...
          fun writeNrrd {name, ty, kind} = if isDyn ty
                then [
                      ...
                      CL.mkIfThen(
                        CL.mkBinOp(
                          CL.mkApply("nrrd_save_helper", [
                              mkOutFile (name, SOME "-len"),
                              nLengthsV
                            ]),
                          CL.#||,
                          CL.mkApply("nrrd_save_helper", [
                              mkOutFile (name, SOME "-data"),
                              nDataV
                            ])),
                      (* then *)
                        error []
                    ...
                    (* else *)
                      CL.mkIfThen(
                        CL.mkApply("nrrd_save_helper", [mkOutFile (name, NONE), nDataV]),
                      (* then *)
                        error []
                      (* endif *))
                    (* endif *)),
