...
structure GenFem : sig
    ...
    val derivativeAffineCase : {prefixE : string ,prefixA : string, sdim : int , dim : int ,dlevel : int} -> string list
end =  struct
...
(************************************************************** translate coords ***********************************************************)
    ...
    fun invJacob 2  =
        ...
        | invJacob 3  =
            ...
            let
                ...
                val t6 = indent"return(0);\n}\n"
            in
               [name, det,m1,m2,m3,m4,m5,m6,m7,m8,m9,t6]
            end
    ...
    fun JI_Inverse{prefix, dim, avgPos, colAcc, basisJacobian}  =
        ...
        (*load to vectors*)
          ...
          in
          [t1,t2,t4]@t3@entriesB@entriesA@entries@invJ
          end
 ...
 fun helpTranslateCoordinates{prefix, dim, avgPos,basisFunctions} =
   let
	    ...
	    val jInit = ""^float^" J["^(Int.toString(dim))^"]["^(Int.toString(dim))^"] = {"^(String.concatWith "," (List.tabulate(dim, fn x => "{"^(String.concatWith "," (List.tabulate(dim,fn x => "0.0f")))^"}")))^"};"
          ...
          in
              [t1,t3]@t4@[t7,jInit,t8,t9,t10]@ttransform@[t11,t12,t13,t14,t15,t16]
          end
    ...
    fun helpMakeEvaluateDerv{prefix, dlevel, sdim, dim, shapeTest, namespace} =
        let
          ...
          val t3 = indent (if shapeTest 
                        then ((fn_affineDerv prefix dlevel)^"(cell,nodes,b,c,nM,pM,H1,0,1);")
...
(************************************************************** mkeval ***********************************************************)
    fun mkps(d,shape,prefixA, dim, dervType) = let
     ...
     (* header to eval function *)
      fun getEvalHeader(d,prefixA, shape,dim) =
        let
	...
	let
	    val d = dlevel
                        (*statements*)
            val t0 = getEvalHeader(dlevel,prefixA, shape,dim)
 ...
 (************************************************************** derivative code ***********************************************************)
	...
	in
      ...
      fun derivativeAffineCase({prefixA,prefixE, sdim,dim,dlevel}) =
	let
	    val ty = implaceArrayU dim dlevel (*add name*)
            (* t1: contains the array H where we store our results *)
	    val t1 = "void *"^(fn_affineDerv prefixA dlevel)^"(int cell, NodeTy nodes, newposTy b, coordTy c, MappTy nM, FloatMapTy pM," ^(float ^ " H " ^ ty )^",int start,int mult){\n"
