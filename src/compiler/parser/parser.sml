...
*)
  ...
  (* select a parser based on the syntax version *)
    ...
    in
    fun selectParser (errStrm, inStrm) = (case getVersion (errStrm, inStrm)
	   of SOME vers => (case lookup vers
		 of NONE => (
		      Error.error (errStrm, [
  ...
  (* parse a file, returning a parse tree *)
    fun parseFile (errStrm, file) = (case selectParser (errStrm, file)
	   of SOME parser => parser (errStrm, file)
	    | NONE => NONE
	  (* end case *))
