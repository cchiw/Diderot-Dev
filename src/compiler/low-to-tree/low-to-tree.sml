...
(* QUESTION: what about vector result/arguments? *)
 ...
 (*
  ...
  (* translation for simple operations (e.g., scalar operations) *)
    ...
    fun FEMsimpleArgs (env, args) = let
         val FEMsimpleArg = FEMsimpleArg env
         fun doArg (arg, (es, stms)) = let
            val (e', stms') = FEMsimpleArg (arg, stms)
            in
                (e'::es, stms')
            end
         in
  ...
  (* translation for simple vector operations *)
    ...
    fun trOp (env, srcRator, args) = let
         ...
         fun FEMbindTREE rator = let
            val (args, stms) = FEMsimpleArgs (env, args)
            in
                (Env.TREE(T.E_Op(rator, args)), stms)
            end
          fun bindTREE' rator = let
...
(* QUESTION: should we just use VAdd 1, etc ?*)
              ...
              | Op.Dimension  => FEMbindTREE(TOp.Dimension)
              | Op.NumCells  => FEMbindTREE(TOp.NumCells)
	      | Op.GetTracker  => FEMbindTREE(TOp.GetTracker)
              | Op.BasisData  =>  FEMbindTREE(TOp.BasisData)
              | Op.CellToNode  => FEMbindTREE(TOp.CellToNode)
              | Op.NodeToPoint  => FEMbindTREE(TOp.NodeToPoint)
              | Op.NodeToCoord  => FEMbindTREE(TOp.NodeToCoord)
              | Op.Coordinates  => FEMbindTREE(TOp.Coordinates)
              | Op.makeTranslateCoordinates e => FEMbindTREE (TOp.makeTranslateCoordinates e)
              | Op.makeFindCellExpand e => FEMbindTREE (TOp.makeFindCellExpand  e)
              | Op.makeBasisEvaluation e =>FEMbindTREE  (TOp.makeBasisEvaluation e)
              | Op.makeFindCellPush dim   => FEMbindTREE(TOp.makeFindCellPush(dim))
              | Op.GetCell   => FEMbindTREE(TOp.GetCell)
              | Op.GetPos   => FEMbindTREE(TOp.GetPos)
              | Op.GetNode  => FEMbindTREE(TOp.GetNode)
              | Op.ProbeNodeB => FEMbindTREE(TOp.ProbeNodeB)
              | Op.ProbeNodeC => FEMbindTREE(TOp.ProbeNodeC)
              | Op.ProbeF es => FEMbindTREE(TOp.ProbeF es)
              | Op.ProbeInvF es => FEMbindTREE(TOp.ProbeInvF es)
              | Op.ProbePhi es => FEMbindTREE(TOp.ProbePhi es)
              | Op.EvalFem es => FEMbindTREE(TOp.EvalFem es )
