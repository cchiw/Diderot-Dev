...
structure Util : sig
    ...
    fun trType ty =let
        val ty' = (case ty
           of LowTypes.BoolTy => TreeTypes.BoolTy
            ...
            | LowTypes.arrTy(e1)=> TreeTypes.arrTy(trType e1)
                (*FEm src field*)
            | LowTypes.FemFldTy => TreeTypes.FemFldTy
                (* other fem data*)
            | LowTypes.BasisDataTy  => TreeTypes.BasisDataTy
            | LowTypes.polyTy => TreeTypes.polyTy
            | LowTypes.coordTy => TreeTypes.coordTy
            | LowTypes.fcTy=> TreeTypes.fcTy
            | LowTypes.jacobiansTy => TreeTypes.jacobiansTy
            | LowTypes.JITy => TreeTypes.JITy
            | LowTypes.transformsTy => TreeTypes.transformsTy
            | LowTypes.MappTy(e1,e2)=> TreeTypes.MappTy(trType e1, trType e2)
	    | LowTypes.optStruct => TreeTypes.optStruct
          (* end case *))
        (*val _ = print(String.concat["\n cvt:",LowTypes.toString(ty),"-->",TreeTypes.toString(ty')])*)
        in ty' end
  ...
  (* for parameters, we use tensor refs to avoid copying and strand pointers.
   * Note that user-defined functions cannot have strand parameters, so this translation
   * only affects map functions.
   *)
    fun trParamTy (ty as LowTypes.TensorTy(shp as _::_)) = TreeTypes.TensorRefTy shp
      | trParamTy (LowTypes.StrandTy n) = TreeTypes.StrandIdTy n
      | trParamTy ty = trType ty
    ...
    fun toAPIType ty = (case ty
           of LowTypes.BoolTy => APITypes.BoolTy
            ...
            | LowTypes.FemFldTy => APITypes.StringTy
    ...
    val newParamVar = newVar trParamTy "p_"
                ...
                in
                  case stm
                   of IR.S_Assign(_, _, e) => next (chkExp (e, uG, nW))
                    ...
                    | IR.S_Return e => next (chkExp (e, uG, nW))
                    | _ => next (uG, nW)
                  (* end case *)
                end
