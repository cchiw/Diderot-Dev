structure ScanPP : sig
   ...
   (* conversion for different operations to strings *)
    ...
    fun mkOrdered(op_n, s1, alpha1) = (case alpha1
        ...
        | [E.V v1, E.V v2] =>
            if(v1<v2) 
              then (s1, alpha1)
              else  (mkTranspose(op_n, s1), [E.V v2, E.V v1])
...
(* ------------------------------------------------------------------ *)
   ...
   (* subtraction operation *)
    fun rewriteOp_subtraction(op_n, e, s1, alpha1, s2, alpha2) = 
                ...
                | E.Op2(E.Sub, 
                    ...
                    in
                        ...
                        then (mkDet2(cvtId(args, ida)), [])
                            ...
                            val (s1, alpha1) = getAlpha (e1)
                            val (s2, alpha2) = getAlpha (e2)
                            in rewriteOp_subtraction(op_n, e, s1, alpha1, s2, alpha2) end
                    end
                | E.Op2(E.Sub, 
                ...
                | E.Op2(op2, e1, e2) =>  let
                    ...
                    val (s2, alpha2) = getAlpha (e2)
                     val (name, beta) = (case op2
                        of E.Sub => rewriteOp_subtraction(op_n, e, s1, alpha1, s2, alpha2)
