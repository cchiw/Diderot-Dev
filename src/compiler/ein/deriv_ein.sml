    ...
    fun differentiate (px, body) =
        ...
        val body' =(case body
            ...
            | E.Opn(opn, es)        =>             let
                ...
                in iterAA(xx) end
            | E.EvalFem(fnspace, [phi, inv, pos]) => let
                val phi' = differentiate(px, phi)
                val inv' = differentiate(px, inv)
                val e1  = E.EvalFem(fnspace, [phi', inv, pos])
                val e2  = E.EvalFem(fnspace, [inv', pos])
                val newbie = E.Opn(E.Prod, [e1, e2])
                in newbie end
            | E.EvalFem(fnspace, [inv, pos]) => let
                val inv' = differentiate(px, inv)
                in E.EvalFem(fnspace, [inv', pos]) end
            | E.Basis(fnspace, id, dx) => E.Basis(fnspace, id, dx@px)
            | E.BigF(fnspace, id, dx) => E.BigF(fnspace, id, dx@px)
            | E.Inverse(fnspace, f) => let
