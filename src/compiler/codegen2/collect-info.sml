    ...
    fun addOp info = let
          ...
          fun add' rator = (case rator
                 of Op.RClamp => insert RClamp
                  ...
                  | Op.EvalFem (space,nnodes,level,shape) => (case (level,shape)
                    of (0,[])=>  (insert(ProbePhi (space,nnodes,0,0,0));insert(EvalFemSca (space,nnodes,0,true)))
                    | (_, []) => (insert(EvalFemSca (space,nnodes,level,true)))
                    | (0,_) =>   (insert(ProbePhi (space,nnodes,0,0,0));insert(EvalFemSca (space,nnodes,0,false));insert(EvalFemShape (space,nnodes,0,shape)))
                    |(_,_) =>    (insert(EvalFemSca (space,nnodes,level,false));insert(EvalFemShape (space,nnodes,level,shape)))
                    (*end case*))
