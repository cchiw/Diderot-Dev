...
structure MidToLow : sig
    ...
    fun cvtTy ty = (case ty
           of SrcTy.BoolTy => DstTy.BoolTy
            ...
            (*FEM data types*)
            | SrcTy.FemFldTy => DstTy.FemFldTy
            (*remove these types*)
            | SrcTy.MeshTy  => DstTy.intTy
            | SrcTy.ElementTy => DstTy.intTy
            | SrcTy.FnSpaceTy => DstTy.intTy
            (*Results of data types*)
            | SrcTy.BasisDataTy  => DstTy.BasisDataTy
            | SrcTy.polyTy => DstTy.polyTy
            | SrcTy.coordTy => DstTy.coordTy
            | SrcTy.fcTy=> DstTy.fcTy
            | SrcTy.jacobiansTy => DstTy.jacobiansTy
            | SrcTy.JITy => DstTy.JITy
            | SrcTy.transformsTy => DstTy.transformsTy
            | SrcTy.MappTy(e1,e2)=> DstTy.MappTy(cvtTy e1,cvtTy e2)
            | SrcTy.arrTy(e1)=> DstTy.arrTy(cvtTy e1)
	    | SrcTy.optStruct => DstTy.optStruct
          (* end case *))
...
(* QUESTION: will we still have BorderCtl* operators at this point? *)
              ...
              | SrcOp.BuildMesh  _ => dummy ()
              | SrcOp.BuildElement _  => dummy ()
              | SrcOp.BuildSpace  => dummy ()
              
              | SrcOp.Dimension  => assign DstOp.Dimension
              | SrcOp.NumCells  => assign DstOp.NumCells
	      | SrcOp.GetTracker => assign DstOp.GetTracker
              | SrcOp.BasisData  => assign DstOp.BasisData
              | SrcOp.CellToNode  => assign DstOp.CellToNode
              | SrcOp.NodeToPoint  => assign DstOp.NodeToPoint
              | SrcOp.NodeToCoord  => assign DstOp.NodeToCoord
              | SrcOp.Coordinates  => assign DstOp.Coordinates
              | SrcOp.makeTranslateCoordinates e  => assign (DstOp.makeTranslateCoordinates e)
              | SrcOp.makeFindCellExpand e => assign (DstOp.makeFindCellExpand e)
              | SrcOp.makeBasisEvaluation e => assign (DstOp.makeBasisEvaluation e)
              | SrcOp.makeFindCellPush dim   => assign(DstOp.makeFindCellPush dim)
              | SrcOp.GetCell   => assign DstOp.GetCell
              | SrcOp.GetPos   => assign DstOp.GetPos
              | SrcOp.GetNode  => assign DstOp.GetNode 
              | SrcOp.ProbeNodeB => assign DstOp.ProbeNodeB
              | SrcOp.ProbeNodeC => assign DstOp.ProbeNodeC
