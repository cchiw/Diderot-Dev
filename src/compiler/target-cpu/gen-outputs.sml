...
structure GenOutputs : sig
  ...
  (* error message for no active/stable strands *)
    fun noStrandsMsg snapshot = let
          val msg = if snapshot
                then "no alive strands, so no snapshot"
                else "no stable strands at termination, so no output"
          in
            CL.mkCallExp(CL.mkIndirect(wrldV, "error"), [CL.mkStr msg])
          end
   ...
   *)
    fun genDynOutput (env, snapshot, nAxes, ty, name, kind) = let
        ...
        (* code to check for zero outputs, which happens for collections with no strands *)
          val checkForNoStrands = if #isGrid spec
                then []
                else [
                    CL.mkComment["check for no output"],
                    CL.mkIfThen(
                      CL.mkBinOp(mkInt 0, CL.#==, numStrandsExp snapshot),
                      CL.mkBlock[
                          noStrandsMsg snapshot,
        ...
        (* code to check for no data to output (i.e., all of the output sequences are empty) *)
          val checkForEmpty = [
                  ...
                  CL.mkIfThen(
                    ...
                    CL.mkBlock[
(* FIXME: this approach does not work, since it causes the write to fail *)
                        CL.mkCall("nrrdEmpty", [nDataV]),
   ...
   *)
    fun genFixedOutput (env, snapshot, nAxes, ty, name, kind) = let
        ...
        (* code to check for no data to output (i.e., no alive strands) *)
