    ...
    fun genInitInputs (env, inputs) = let
         ...
         *)
          fun initInput (Inp.INP{var, name, ty, desc, init}, stms) = (case ty
                 of Ty.SeqTy(_, NONE) =>
                      ToC.loadNrrd (global var, CL.mkIndirect(inpV, GVar.qname var)) :: stms
                  | Ty.ImageTy _ =>
                      ToC.loadNrrd (global var, CL.mkIndirect(inpV, GVar.qname var)) :: stms
                  | ty => U.copy{
                ...
                in
                  case init
                   of Inp.NoDefault => mkIfStm(undef ())
                    ...
                    | Inp.LoadSeq file => mkIfStm(ToC.loadNrrd (global var, CL.mkStr file))
                    | Inp.Proxy(file, _) => mkIfStm(ToC.loadNrrd (global var, CL.mkStr file))
                ...
                val setDcl = (case ty
                       of Ty.ImageTy _ => [
                              cFunc(
                                ...
                                CL.mkBlock[
                                    wrldCastStm,
                                    ToC.loadNrrd (global var, CL.mkVar "s"),
                              ...
                              cFunc(
                                ...
                                CL.mkBlock[
                                    wrldCastStm,
                                    ToC.loadNrrd (global var, CL.mkVar "nin"),
                                    CL.mkReturn(SOME(CL.mkBool false))
                                  ])
                            ]
