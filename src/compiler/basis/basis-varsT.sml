...
(* ----------------------------------------------------------------------------------*)
    ...
    val fn_udf_11 = polyVar(N.fn_udf , all([SK, DK, NK, SK, SK],
        fn [Ty.SHAPE ddT0, Ty.DIFF k, Ty.DIM d, Ty.SHAPE ddF, Ty.SHAPE ddT1] => let
            val t0 = Ty.T_Tensor(Ty.ShapeVar ddT0)
            val t1 = Ty.T_Tensor(Ty.ShapeVar ddT1)
            val texp = Ty.T_Tensor(Ty.ShapeVar ddF)
            val k0 = Ty.DiffVar(k, 0)
            val f1 = Ty.T_Field{diff = k0, dim = Ty.DimVar d, shape = Ty.ShapeVar ddF}
        in
            [t0,texp, t1] --> f1
