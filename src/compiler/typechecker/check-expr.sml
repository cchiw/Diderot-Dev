...
(* FIXME: need kind for concrete types here! *)
 ...
 (* older version: pointer to field data
         | PT.E_Mesh x => (case Env.findVar ( env, x)
            of SOME x' => let
                val (tyArgs, Ty.T_Fun(_, rngTy)) = TU.instantiate(Var.typeOf(BV.mesh))
                val (xx, ty1) = check (env, cxt, PT.E_Var x)
                val (m,_) = (case xx
                    of AST.E_Var l => useVar(cxt, l)
                    (* end case*))
                in (case rngTy
