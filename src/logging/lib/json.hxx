...
namespace JSON {
  ...
  // some helper functions that chain null testing with type coercions
    inline const Object *asObject (const Value *v)
    {
	return (v == nullptr) ? nullptr : v->asObject();
    }
    inline const Array *asArray (const Value *v)
    {
	return (v == nullptr) ? nullptr : v->asArray();
    }
    inline const Number *asNumber (const Value *v)
    {
	return (v == nullptr) ? nullptr : v->asNumber();
    }
    inline const String *asString (const Value *v)
    {
	return (v == nullptr) ? nullptr : v->asString();
    }
    inline const Bool *asBool (const Value *v)
    {
	return (v == nullptr) ? nullptr : v->asBool();
    }
