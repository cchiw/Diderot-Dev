...
/******************** 1D images ********************/
    ...
    bool image1d<REAL,TY>::load (struct world_base *wrld, std::string const &name)
    {
    ...
    template <typename REAL, typename TY>
    bool image1d<REAL,TY>::load (struct world_base *wrld, const Nrrd *nin)
    {
      // compute the offset to the first space axis
        int base = nin->dim - nin->spaceDim;

// FIXME: we should also be checking the shape of the voxels and the dimensions
      // check that the image format is what we expect
        if (nin->spaceDim != 1) {
            wrld->error ("nrrd has unexpected dimension %d, expected 1\n", nin->spaceDim);
            return true;
        }
        else if (nin->axis[base].kind != nrrdKindSpace) {
            wrld->error ("nrrd has unexpected axis structure: %s\n",
                airEnumStr(nrrdKind, nin->axis[base].kind));
            return true;
        }
        else if (nin->type != image1d<REAL,TY>::traits::type) {
            wrld->error ("nrrd has unexpected sample type %s; expected %s\n",
                ((0 <= nin->type) && (nin->type < nrrdTypeLast))
                    ...
                    : "unknown",
                __details::nrrd_type_info[image1d<REAL,TY>::traits::type].name);
            return true;
        }
...
/******************** 2D images ********************/
    ...
    template <typename REAL, typename TY>
    bool image2d<REAL,TY>::load (struct world_base *wrld, std::string const &name)
    {
        Nrrd *nin = wrld->load_nrrd_file (name);
        if (nin == nullptr)
            return true;
    ...
    template <typename REAL, typename TY>
    bool image2d<REAL,TY>::load (struct world_base *wrld, const Nrrd *nin)
    {
      // compute the offset to the first space axis
        int base = nin->dim - nin->spaceDim;
      ...
      // check that the image format is what we expect
        if (nin->spaceDim != 2) {
            wrld->error ("nrrd has unexpected dimension %d, expected 2\n", nin->spaceDim);
            return true;
        }
        else if ((nin->axis[base].kind != nrrdKindSpace)
        ||       (nin->axis[base+1].kind != nrrdKindSpace)) {
            wrld->error ("nrrd has unexpected axis structure: %s %s\n",
                airEnumStr(nrrdKind, nin->axis[base].kind),
                airEnumStr(nrrdKind, nin->axis[base+1].kind));
            return true;
        }
        else if (nin->type != image2d<REAL,TY>::traits::type) {
            wrld->error ("nrrd has unexpected sample type %s; expected %s\n",
                ((0 <= nin->type) && (nin->type < nrrdTypeLast))
                    ? __details::nrrd_type_info[nin->type].name
                    : "unknown",
                __details::nrrd_type_info[image2d<REAL,TY>::traits::type].name);
            return true;
        }
...
/******************** 3D images ********************/
    ...
    template <typename REAL, typename TY>
    bool image3d<REAL,TY>::load (struct world_base *wrld, std::string const &name)
    {
        Nrrd *nin = wrld->load_nrrd_file (name);
        if (nin == nullptr)
            return true;
    ...
    template <typename REAL, typename TY>
    bool image3d<REAL,TY>::load (struct world_base *wrld, const Nrrd *nin)
    {
      // compute the offset to the first space axis
        int base = nin->dim - nin->spaceDim;
      ...
      // check that the image format is what we expect
        if (nin->spaceDim != 3) {
            wrld->error ("nrrd has unexpected dimension %d, expected 3\n", nin->spaceDim);
            return true;
        }
        else if ((nin->axis[base].kind != nrrdKindSpace)
        ||       (nin->axis[base+1].kind != nrrdKindSpace)
        ||       (nin->axis[base+2].kind != nrrdKindSpace)) {
            wrld->error ("nrrd has unexpected axis structure: %s %s %s\n",
                airEnumStr(nrrdKind, nin->axis[base].kind),
                airEnumStr(nrrdKind, nin->axis[base+1].kind),
                airEnumStr(nrrdKind, nin->axis[base+2].kind));
            return true;
        }
        else if (nin->type != image3d<REAL,TY>::traits::type) {
            wrld->error ("nrrd has unexpected sample type %s; expected %s\n",
                ((0 <= nin->type) && (nin->type < nrrdTypeLast))
                    ? __details::nrrd_type_info[nin->type].name
                    : "unknown",
                __details::nrrd_type_info[image3d<REAL,TY>::traits::type].name);
            return true;
        }
